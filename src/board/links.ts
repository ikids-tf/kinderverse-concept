...
/* 요소 연결(links) 헬퍼 — 포트 드래그로 만든 from→to 연결의 '순번 라벨'과
   '연결망'을 계산한다.
   라벨 규칙: 머리(들어오는 선 없음)가 1, 첫 연결을 따라 2, 3… 으로 증가.
   이미 연결된 요소에서 '추가로' 가지를 치면 가지의 첫 요소는 1-1, 2-1처럼
   부모 라벨 + "-k" (가지 연속은 1-1 → 1-2 → …). 수업 모드/슬라이드 쇼는
   이 라벨의 계층 숫자 순(1, 1-1, 1-2, 2, 2-1, 3…)으로 정렬한다. */
...
export function linkSequence(links: BoardLink[]): Map<string, string> {
  ...
  const visit = (start: string) => {
    ...
    while (queue.length) {
      ...
      kids.forEach((k, i) => {
        // 첫 연결 = 같은 줄기 계속(2, 3…), 추가 연결 = 가지(1-1, 1-2…)
        labels.set(k, claim(i === 0 ? incLast(L) : `${L}-${i}`));
        queue.push(k);
      });
    }
  };
