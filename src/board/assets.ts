...
const LAND_ANIMALS = ['토끼', '여우', '곰', '곰돌이', '사자', '호랑이', '코끼리', '하마', '기린', '강아지', '개', '고양이', '다람쥐', '원숭이', '판다', '너구리', '사슴', '양', '염소', '돼지', '소', '말', '얼룩말', '코알라', '캥거루', '낙타', '늑대', '두더지', '고슴도치', '햄스터', '쥐', '생쥐', '두더쥐'];
const BIRDS = ['새', '참새', '까치', '비둘기', '파랑새', '부엉이', '올빼미', '독수리', '앵무새', '제비', '닭', '병아리', '오리', '거위', '백조', '학', '플라밍고', '펭귄', '공작', '타조'];
const BUGS = ['나비', '잠자리', '무당벌레', '개미', '벌', '꿀벌', '메뚜기', '사슴벌레', '장수풍뎅이', '딱정벌레', '거미', '달팽이', '애벌레', '귀뚜라미', '매미', '반딧불이'];
const DINOS = ['공룡', '티라노', '티라노사우루스', '브라키오', '브라키오사우루스', '트리케라', '트리케라톱스', '스테고', '스테고사우루스', '프테라', '프테라노돈'];
const SEA = ['물고기', '문어', '오징어', '고래', '상어', '게', '꽃게', '새우', '거북', '거북이', '돌고래', '불가사리', '해마', '조개', '가오리', '복어', '흰동가리', '해파리', '굴', '소라', '전복'];
...
/** 태그를 검색용 '단어'들로 — 괄호·숫자 제거 후 공백 분리, 끝 조사 제거. 카테고리 멤버는 이 단어와
    '정확 일치'로만 매칭해, 짧은 멤버('무')가 '무당벌레' 같은 다른 단어에 부분일치하지 않게 한다. */
function tagWords(tag: string): string[] {
  return tag
    .replace(/\([^)]*\)/g, ' ') // (생성)·(배경제거) 등 제거
    .replace(/\d+/g, ' ') // 숫자 제거('2 적힌 수박' → 적힌 수박)
    .split(/[\s,·!?]+/)
    .map((w) => norm(w.replace(/(을|를|이|가|은|는|에|의|로|와|과|만|도)$/u, '')))
    .filter((w) => w.length >= 1);
}
