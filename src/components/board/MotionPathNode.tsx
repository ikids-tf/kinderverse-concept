...
function getP(n: BoardNode, k: 'p1' | 'p2' | 'c1' | 'c2'): P {
...
function bz(t: number, a: number, b: number, c: number, d: number): number {
  const u = 1 - t;
  return u * u * u * a + 3 * u * u * t * b + 3 * u * t * t * c + t * t * t * d;
}

/** 노드의 시각 중심(스케일은 중심 기준이라 x+w/2가 그대로 중심). */
function centerOf(n: BoardNode): P {
...
/** 점 드래그/연결 카드 이동 후 — 점들의 헐(bbox)로 모션 노드 박스를 다시 감싼다.
    BoardCanvas(연결 카드 드래그 후 출발점 동기화)와 컴포넌트가 함께 쓴다. */
