        ...
        {beh ? (
          <div className="flex flex-col gap-2">
            ...
            <label className="flex items-center gap-2 px-1 text-[11px] text-fg-2">
              ...
              <select
                value={
                  beh.trigger === 'sceneEnter'
                    ? 'enter'
                    : beh.trigger === 'sequenceTap'
                      ? 'seq'
                      : beh.trigger === 'storyAdvance'
                        ? 'story'
                        : beh.trigger === 'pathTraverse'
                          ? 'drag'
                          : 'tap'
                }
                onChange={(e) => {
                  const v = e.target.value;
                  onSetTrigger(v === 'enter' ? 'sceneEnter' : v === 'seq' ? 'sequenceTap' : v === 'story' ? 'storyAdvance' : v === 'drag' ? 'pathTraverse' : 'tap');
                }}
              ...
              >
                ...
                <option value="drag">끌어서 잇기 (연결로 드래그)</option>
                <option value="story">이야기 넘길 때</option>
              </select>
            </label>
