...
const SCENE_STYLE =
  ...
  '★화면 하단에는 인물·아이템이 올라설 분명한 바닥면(땅·잔디·마루·길·모래 등 주제에 맞는 지면)을 두어, 그 위에 놓일 캐릭터가 공중에 뜨지 않고 바닥에 발을 디딘 것처럼 보이게 한다(하단 전체를 허공·하늘·물 등 설 수 없는 면으로 비우지 말 것 — 하단 1/3은 닿을 수 있는 지면)';
