...
/** 이미지 선택 + "배경 제거/누끼/투명 배경" → 배경 지운 투명 PNG로 변환 + 갤러리 저장.
    "투명 배경으로 ○○ 그려줘"(생성+투명)도 이 패턴으로 잡아 prompt.ts가 생성→자동 누끼로 보낸다.
    조사 변형(배경"이"/"가"/"은"/"는" 제거된·없는·투명한)과 "PNG로"까지 폭넓게 매칭한다. */
export const BG_REMOVE_RE =
  /누끼|배경\s*(이|가|은|는|을|를|도|만)?\s*(제거|삭제|지우|지워|지운|없애|없애줘|없이|없는|없게|빼고|빼|뺀|날려|날리|날린|투명|딴|따)|투명\s*(한|하게|으로)?\s*배경|투명\s*(하게|으로|png)|배경\s*투명|뒷?배경\s*(제거|삭제|없이|없는|투명)|\bpng\s*(파일|이미지)?\s*(로|으로)/i;
