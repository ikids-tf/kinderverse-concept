...
/* 투명 컷아웃용 — "투명 배경에 ○○ 그려줘". 누끼(RMBG)가 깨끗하게 떨어지도록 단색 순백
   배경에 단일 오브젝트만 또렷이 생성한다(장식 배경은 모델이 전경으로 오인해 안 지워짐). */
export const KV_CUTOUT_STYLE =
  '밝고 따뜻한 유아 그림책 일러스트 스타일, 부드러운 파스텔 색감, 둥근 형태; 화면 정중앙에 단 하나의 대상만 크고 또렷하게, 완전한 순백색(#FFFFFF) 단색 배경, 배경에 무늬·풍경·바닥선·그림자·테두리·소품 전혀 없음, 대상의 외곽선이 배경과 뚜렷이 분리됨, 스티커처럼 한 오브젝트만 고립되게, 유아가 무서워할 요소 없음, 이미지 안에 글자·숫자·문자 절대 없음';
