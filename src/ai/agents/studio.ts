...
/* 투명 컷아웃용 — "투명 배경에 ○○ 그려줘". 누끼(RMBG)가 한 번에 깨끗이 떨어지도록
   "매트(matting)하기 가장 쉬운 그림"이 되는 조건을 강하게 박아 넣는다: 균일한 단색 배경 +
   고대비 + 그림자·그라데이션·무늬 없음 + 단일 오브젝트 + 넉넉한 여백 + 또렷한 닫힌 외곽선.
   (장식 scene을 그리면 모델이 배경을 전경으로 오인해 누끼 후 배경에 지저분한 잔여물이 남는다.) */
export const KV_CUTOUT_STYLE =
  '밝고 따뜻한 유아 그림책 일러스트 스타일, 부드러운 파스텔 색감, 둥근 형태. ' +
  '그리는 대상은 단 하나뿐 — 소품·배경 사물·풍경·텍스트를 절대 넣지 않는다. ' +
  '대상은 화면 정중앙에 전체가 온전히 보이도록 배치하고, 네 가장자리에서 넉넉히 떨어뜨려 충분한 여백을 남긴다(잘리거나 테두리에 닿지 않게). ' +
  '배경은 처음부터 끝까지 완벽하게 평평하고 균일한 단 하나의 단색으로만 채운다 — 기본은 순백색(#FFFFFF), 단 대상이 희거나 아주 옅으면 대상과 또렷이 대비되는 단색 파스텔. ' +
  '그라데이션·비네팅·질감·무늬·패턴·테두리·액자·바닥선·소품·그림자·반사·빛번짐을 배경에 절대 넣지 않는다(대상 아래 그림자 금지). ' +
  '대상의 윤곽선은 또렷하고 닫혀 있으며 배경과 색·명도가 분명히 달라 경계가 선명하다. ' +
  '대상 내부는 불투명한 단색 면으로 채워 반투명·흐릿한·뿌연 가장자리가 없게 한다. ' +
  '유아가 무서워할 요소 없음, 이미지 안에 글자·숫자·문자 절대 없음.';
