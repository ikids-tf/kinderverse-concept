...
export function buildWeeklyPlanDoc(payload) {
  ...
  // 표 구조(테두리·분리선·배경 = shape)만 고정(locked)해 레이아웃을 지키고,
  // 텍스트와 이미지(스티커)는 편집 가능하게 둔다 → 교사가 내용 수정 + 스티커 이동/재생성.
  // (월안 템플릿과 동일한 편집 자유도 — 이전엔 이미지까지 잠겨 편집 불가였음)
...
export function isTemplateId(id) {
  if (id === "topicweb" || id === "weeklyplan" || id === "weeklyplan-journal" || id === "monthlyplan" || id === "monthlyplan-summer") return true;
  const [theme, family] = String(id).split("-");
