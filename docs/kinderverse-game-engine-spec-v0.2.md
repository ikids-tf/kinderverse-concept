...
> - **메커니즘 명칭은 코드와 일치(10종이 아니라 11종)**: `sequence-order`·`path-trace`·`pair-match`·`tap-select`·`branch-choose`·`combine`·`memory-flip`·`free-create`·`sort-to-bin`·`slot-fill`·`dress-up`(옷입히기·날씨 옷차림) 모두 B에 구현됨(§4 표 유효). `dress-up`은 `recipeTypes.ts`의 `MechanismId`, `index.ts`의 `RECIPES`, `recipes/dressUp.ts`, `selectRecipe` 매핑에 존재. §4-1 드래그 모델 조사는 해소됨(`sort-to-bin`·`slot-fill` 구현 확인).
...
| **sort-to-bin** | 분류·나누기 | items→bins, 연결-구속 드래그(각 item→정답 통 path; 타깃≥2종 시 드래그-분류 활성), `recipes/dragSort.ts` | ◐ 조합 |
| **slot-fill** | 빈칸 완성 | piece→blank, 연결-구속 드래그(각 조각→정답 빈칸 path), `recipes/dragSort.ts` | ◐ 조합 |
| **dress-up** | 옷입히기·날씨 옷차림 | 착장별 캐릭터 `reveal`/`hide` + `tap`/`pathTraverse`(옷→어린이 연결); 실내↔실외 배경 전환, `recipes/dressUp.ts` | ◐ 조합 |
...
### 4-1. (해소됨) 드래그 모델 — 연결-구속으로 확인 (sort-to-bin · slot-fill)

> **해소됨(2026-06-26):** 드래그 모델은 free-drop이 아닌 **연결-구속(`pathTraverse`)** 으로 확인. `sort-to-bin`·`slot-fill` 모두 `recipes/dragSort.ts`로 **구현 완료**. 아래는 결정 당시의 조사 맥락(이력).
...
→ **해소됨:** 드래그 모델은 free-drop이 아닌 **연결-구속(`pathTraverse`)** 로 확인됐고, `sort-to-bin`·`slot-fill` 모두 `recipes/dragSort.ts`로 **구현 완료**(§10 조사 항목 1 해소).
...
```
ThemePack { id: string; names: string[]; vocabulary: string[]; scene: string; season?: 'spring'|'summer'|'autumn'|'winter' }
```

> 구현 일치(`themePacks.ts:9-19`): `maps{lifeTopics[],seasons[]}`·`tokens` 필드는 없다 — `names`(식별 이름·동의어)·`scene`(배경 프롬프트)·`season`(계절 폴백)으로 구성. (파스텔 child 토큰은 ThemePack이 아니라 `theme.ts` 소관.)
...
> **2026-06-26 기준 Resolver(`src/features/interactive-viewer/resolver/`)에 구현 완료. 역사적 참고용.** 아래 조사 프롬프트는 구현 착수 전 작성된 것으로, 결론은 이미 코드에 반영됐다.
...
조사 항목:
1. ★ 드래그 모델 [해소됨 — 연결-구속(pathTraverse) 확인, sort-to-bin·slot-fill 모두
   recipes/dragSort.ts 구현 완료] — pathTraverse가 "연결을 따라"만 되는 연결-구속인가,
   아니면 임의 위치 자유 드롭 + 영역 판정(free-drop)이 가능한가? onPathUp(877~) 로직 확인.
...
## 11. Claude Code 핸드오프 (완료 — 이력)

> **2026-06-26 기준 Resolver(`src/features/interactive-viewer/resolver/`)에 구현 완료. 역사적 참고용.** 아래 핸드오프 계획은 이미 실행됐다.
